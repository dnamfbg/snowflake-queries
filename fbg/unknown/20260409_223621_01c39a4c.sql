-- Query ID: 01c39a4c-0212-67a8-24dd-07031945b52f
-- Database: unknown
-- Schema: unknown
-- Warehouse: BI_SER_XL_WH
-- Executed: 2026-04-09T22:36:21.073000+00:00
-- Elapsed: 246ms
-- Environment: FBG

SELECT SYSTEM$VALIDATE_QUERY('WITH market_map AS (
...
base AS (
    SELECT
        ...
        t.TRADING_WIN,
        CASE
            WHEN p.DRAFTKINGS IS NULL OR p.DRAFTKINGS = 0 THEN NULL
            ELSE t.TOTAL_CASH_STAKE_BY_LEGS
                - (t.TOTAL_CASH_WAGER_PAYOUT_BY_LEGS * (p.DRAFTKINGS / NULLIF(p.FBG, 0)))
        END AS hypothetical_trading_win_at_draftkings
    FROM market_map m
...
SELECT
    clock.PERIOD_NUMBER,
    CASE
        WHEN DRAFTKINGS = 0 THEN ''DK Suspended''
        WHEN FBG > DRAFTKINGS THEN ''FBG Above DraftKings''
        WHEN FBG < DRAFTKINGS THEN ''FBG Below DraftKings''
        ELSE ''FBG Equal to DraftKings''
    END AS fbg_vs_draftkings,
    ...
    SUM(hypothetical_trading_win_at_draftkings)
        / NULLIF(SUM(CASE WHEN DRAFTKINGS != 0 THEN total_cash_stake_by_legs END), 0) AS DK_TR_Margin,
    SUM(hypothetical_trading_win_at_draftkings) AS DK_TR_Win,

    SUM(trading_win) - SUM(hypothetical_trading_win_at_draftkings) AS TR_Win_Difference,
    COUNT(DISTINCT CASE WHEN DRAFTKINGS = 0 THEN wager_id END) AS Suspended_Count
FROM base b
JOIN FBG_ANALYTICS.TRADING.SPORT_RADAR_NBA_PBP_PLAYS pl
    ON b.EVENT_ID = pl.NATS_ID
    AND pl.SINGLE_NATS_ID_FILTER = 1
    AND pl.PLAY_ID_WALL_CLOCK_END_ >= pl.PLAY_ID_WALL_CLOCK_START_
JOIN FBG_ANALYTICS.TRADING.SPORT_RADAR_NBA_PBP_GAME_CLOCK_WINDOWS clock
    ON pl.PLAY_ID = clock.PLAY_ID
    AND b.WAGER_PLACED_TIME_UTC BETWEEN clock.PLAY_START_TIME AND clock.PLAY_END_TIME
GROUP BY ALL
ORDER BY clock.PERIOD_NUMBER, fbg_vs_draftkings')
