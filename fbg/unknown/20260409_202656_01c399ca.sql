-- Query ID: 01c399ca-0212-644a-24dd-07031928b2f3
-- Database: unknown
-- Schema: unknown
-- Warehouse: BI_SER_XL_WH
-- Executed: 2026-04-09T20:26:56.393000+00:00
-- Elapsed: 1437ms
-- Environment: FBG
...
base AS (
    SELECT
        ...
        t.TRADING_WIN,
        CASE
            WHEN p.FANDUEL IS NULL OR p.FANDUEL = 0 THEN NULL
            ELSE t.TOTAL_CASH_STAKE_BY_LEGS
                - (t.TOTAL_CASH_WAGER_PAYOUT_BY_LEGS * (p.FANDUEL / NULLIF(p.FBG, 0)))
        END AS hypothetical_trading_win_at_fanduel
    FROM market_map m
...
SELECT
    --prop_type,
    CASE
        WHEN FANDUEL = 0 THEN 'FD Suspended'
        WHEN FBG > FANDUEL THEN 'FBG Above FanDuel'
        WHEN FBG < FANDUEL THEN 'FBG Below FanDuel'
        ELSE 'FBG Equal to FanDuel'
    END AS fbg_vs_fanduel,
    ...
    SUM(hypothetical_trading_win_at_fanduel)
        / NULLIF(SUM(CASE WHEN FANDUEL != 0 THEN total_cash_stake_by_legs END), 0) AS FD_TR_Margin,
    SUM(hypothetical_trading_win_at_fanduel) AS FD_TR_Win,

    SUM(trading_win) - SUM(hypothetical_trading_win_at_fanduel) AS TR_Win_Difference,
    COUNT(DISTINCT CASE WHEN FANDUEL = 0 THEN wager_id END) AS Suspended_Count
FROM base
GROUP BY fbg_vs_fanduel
