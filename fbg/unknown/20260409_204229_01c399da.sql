-- Query ID: 01c399da-0212-67a8-24dd-0703192c0173
-- Database: unknown
-- Schema: unknown
-- Warehouse: BI_M_WH
-- Executed: 2026-04-09T20:42:29.083000+00:00
-- Elapsed: 1706ms
-- Environment: FBG
...
SELECT
    ...
    ROUND(SUM(ABS(actual_fee)) - (SELECT total_refunded FROM refunds), 2) AS net_fees_collected,
    ROUND((SUM(ABS(actual_fee)) - (SELECT total_refunded FROM refunds)) * 0.60, 2) AS fmx_net_collected,
    ...
    ROUND(SUM(fee_at_final) - (SUM(ABS(actual_fee)) - (SELECT total_refunded FROM refunds)), 2) AS gap_final_vs_net,
    ROUND((SUM(fee_at_final) - (SUM(ABS(actual_fee)) - (SELECT total_refunded FROM refunds))) * 0.60, 2) AS fmx_gap_final_vs_net,
