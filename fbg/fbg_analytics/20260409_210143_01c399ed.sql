-- Query ID: 01c399ed-0212-644a-24dd-0703192faedb
-- Database: FBG_ANALYTICS
-- Schema: unknown
-- Warehouse: BI_SER_L_WH
-- Executed: 2026-04-09T21:01:43.086000+00:00
-- Elapsed: 202158ms
-- Environment: FBG
...
SELECT "Custom SQL Query"."BOOST_CASH_USERS" AS "BOOST_CASH_USERS",
  ...
  "Custom SQL Query"."DAILY_CASH_BETS" AS "DAILY_CASH_BETS",
  "Custom SQL Query"."DAILY_CASH_HANDLE" AS "DAILY_CASH_HANDLE",
  "Custom SQL Query"."DAILY_TOTAL_BETS" AS "DAILY_TOTAL_BETS",
  "Custom SQL Query"."DAILY_TOTAL_HANDLE" AS "DAILY_TOTAL_HANDLE",
  ...
  "Custom SQL Query"."PLACED_DATE_ET" AS "PLACED_DATE_ET",
...
FROM (
  ...
  date(min(WAGER_PLACED_TIME_EST)) as First_Bet_Date
  ...
  date(b.WAGER_PLACED_TIME_EST) as Placed_Date_ET,
  sum(case when b.is_Free_bet_wager = 'FALSE' then 1 else 0 end) as Daily_Cash_Bets,
  count(b.account_id) as Daily_Total_Bets,
  sum(b.total_cash_stake_by_legs) as Daily_Cash_Handle,
  sum(b.total_cash_stake_by_legs) + sum(total_non_cash_stake_by_legs) as Daily_Total_Handle,
  ...
  count(distinct case when FB.First_Bet_Date = date(B.WAGER_PLACED_TIME_EST) then b.account_id end) as FTU,
  ...
  (select
  B.account_id,
  date(WAGER_PLACED_TIME_EST) as Placed_Date_ET,
  ...
  sum(Daily_Cash_Bets) as Daily_Cash_Bets,
  sum(Daily_Total_Bets) as Daily_Total_Bets,
  sum(Daily_Cash_Handle) as Daily_Cash_Handle,
  sum(Daily_Total_Handle) as Daily_Total_Handle,
  ...
  FROM Daily_Users DU 
  LEFT JOIN Daily_Boosts DB on DB.account_id = DU.account_id and DB.Placed_Date_ET = DU.Placed_Date_ET
  group by all
