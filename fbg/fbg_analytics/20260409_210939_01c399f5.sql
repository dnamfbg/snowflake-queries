-- Query ID: 01c399f5-0212-6dbe-24dd-0703193205db
-- Database: FBG_ANALYTICS
-- Schema: unknown
-- Warehouse: BI_SER_L_WH
-- Executed: 2026-04-09T21:09:39.255000+00:00
-- Elapsed: 164782ms
-- Environment: FBG
...
SELECT "Custom SQL Query"."BOOST_CASH_USERS" AS "BOOST_CASH_USERS",
  ...
  "Custom SQL Query"."PLACED_WEEK_ET" AS "PLACED_WEEK_ET",
  ...
  "Custom SQL Query"."WEEKLY_CASH_BETS" AS "WEEKLY_CASH_BETS",
  "Custom SQL Query"."WEEKLY_CASH_HANDLE" AS "WEEKLY_CASH_HANDLE",
  "Custom SQL Query"."WEEKLY_TOTAL_BETS" AS "WEEKLY_TOTAL_BETS",
  "Custom SQL Query"."WEEKLY_TOTAL_HANDLE" AS "WEEKLY_TOTAL_HANDLE",
...
FROM (
  ...
  date(trunc(min(WAGER_PLACED_TIME_EST),'week')) as First_Bet_week
  ...
  date(trunc(b.WAGER_PLACED_TIME_EST,'week')) as Placed_week_ET,
  sum(case when b.is_Free_bet_wager = 'FALSE' then 1 else 0 end) as weekly_Cash_Bets,
  count(b.account_id) as weekly_Total_Bets,
  sum(b.total_cash_stake_by_legs) as weekly_Cash_Handle,
  sum(b.total_cash_stake_by_legs) + sum(total_non_cash_stake_by_legs) as weekly_Total_Handle,
  ...
  count(distinct case when FB.First_Bet_week = date(trunc(B.WAGER_PLACED_TIME_EST,'week')) then b.account_id end) as FTU,
  ...
  (select
  B.account_id,
  date(trunc(WAGER_PLACED_TIME_EST,'week')) as Placed_week_ET,
  ...
  count(distinct case when FB.First_Bet_week = date(trunc(WAGER_PLACED_TIME_EST,'week')) then b.account_id end) as FTU,
  ...
  sum(weekly_Cash_Bets) as weekly_Cash_Bets,
  sum(weekly_Total_Bets) as weekly_Total_Bets,
  sum(weekly_Cash_Handle) as weekly_Cash_Handle,
  sum(weekly_Total_Handle) as weekly_Total_Handle,
  ...
  FROM weekly_Users DU 
  LEFT JOIN weekly_Boosts DB on DB.account_id = DU.account_id and DB.Placed_week_ET = DU.Placed_week_ET
  group by all
