-- Query ID: 01c39a27-0212-644a-24dd-0703193d8ac3
-- Database: FBG_SOURCE
-- Schema: unknown
-- Warehouse: BI_XSM_WH
-- Executed: 2026-04-09T21:59:31.323000+00:00
-- Elapsed: 117499ms
-- Environment: FBG
...
FROM (
  ...
  AND TO_DATE(CONVERT_TIMEZONE('UTC', 'America/New_York', b.placed_time)) >= '2026-01-23'
  AND TO_DATE(CONVERT_TIMEZONE('UTC', 'America/New_York', b.placed_time)) <= '2026-02-08'
  ...
  AND j.jurisdiction_code IN ('AZ', 'CT', 'CO', 'IA', 'IL', 'MD', 'MO', 'OH', 'DC', 'VA') -- only certain states
  ...
  WHERE BONUS_CAMPAIGN_ID IN (1037422) -- Promo Campaign IDs
  AND STATE != 'AVAILABLE'
