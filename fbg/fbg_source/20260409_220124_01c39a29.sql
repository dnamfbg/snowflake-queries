-- Query ID: 01c39a29-0212-67a9-24dd-0703193e53df
-- Database: FBG_SOURCE
-- Schema: unknown
-- Warehouse: BI_SM_WH
...
SELECT "Custom SQL Query"."DAY" AS "DAY",
  "Custom SQL Query"."NUMCS" AS "NUMCS",
  "Custom SQL Query"."NUMSUBMISSIONS" AS "NUMSUBMISSIONS",
  "Custom SQL Query"."NUMVERIFICATIONS" AS "NUMVERIFICATIONS",
...
FROM (
  ...
  state,
  count(distinct acco_id) as numsubmissions,
  COUNT_IF(kycformresult = 'Verified' or docv_result = 'Verified' or cs_manual = 1) as NumVerifications,
  COUNT_IF (CS_MANUAL = 1) as numcs,
  --numcs/numverifications,
  --numcs/numsubmissions
  from kyc_final
  where kyc_date >= '2024-01-01'
  group by all
  order by day desc
) "Custom SQL Query"
