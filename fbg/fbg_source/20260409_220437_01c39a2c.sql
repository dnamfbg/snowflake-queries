-- Query ID: 01c39a2c-0212-6dbe-24dd-0703193f0473
-- Database: FBG_SOURCE
-- Schema: unknown
-- Warehouse: BI_SER_L_WH_PROD
-- Executed: 2026-04-09T22:04:37.937000+00:00
-- Elapsed: 71123ms
-- Environment: FBG
...
FROM (
  ...
  stage_1 AS (
      ...
      FROM 
          ...
          LEFT OUTER JOIN fbg_analytics.trading.football_game_weeks_ egw ON 
              DATE(CONVERT_TIMEZONE('UTC','America/Anchorage', bet_parts.event_time)) BETWEEN egw.week_start_date AND egw.week_end_date 
              AND CASE WHEN lower(bet_parts.comp) like '%nfl%' THEN 1 WHEN lower(bet_parts.comp) like '%ncaa%' THEN 2 ELSE 0 END = egw.comp_join_flag
      ...
      WHERE
          accounts.test = 0
          AND
          CASE WHEN lower(bet_parts.comp) like '%nfl%' AND DATE(CONVERT_TIMEZONE('UTC','America/Anchorage', bet_parts.event_time))  BETWEEN '2023-08-01' AND '2024-02-12' THEN 1
               WHEN lower(bet_parts.comp) like '%nfl%' AND DATE(CONVERT_TIMEZONE('UTC','America/Anchorage', bet_parts.event_time))  BETWEEN '2024-07-30' AND '2025-02-10' THEN 1
               WHEN lower(bet_parts.comp) like '%nfl%' AND DATE(CONVERT_TIMEZONE('UTC','America/Anchorage', bet_parts.event_time))  BETWEEN '2025-07-31' AND '2026-02-08' THEN 1
               WHEN lower(bet_parts.comp) like '%ncaa%' AND DATE(CONVERT_TIMEZONE('UTC','America/Anchorage', bet_parts.event_time))  BETWEEN '2023-08-21' AND '2024-01-14' THEN 1
               WHEN lower(bet_parts.comp) like '%ncaa%' AND DATE(CONVERT_TIMEZONE('UTC','America/Anchorage', bet_parts.event_time))  BETWEEN '2023-08-19' AND '2025-01-26' THEN 1 ELSE 0 END = 1
          AND
          lower(sport) = 'american_football'
          AND
