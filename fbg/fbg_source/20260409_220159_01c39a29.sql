-- Query ID: 01c39a29-0212-6dbe-24dd-0703193e2c0b
-- Database: FBG_SOURCE
-- Schema: unknown
-- Warehouse: TABLEAU_XL_PROD
-- Executed: 2026-04-09T22:01:59.191000+00:00
-- Elapsed: 85002ms
-- Environment: FBG
...
FROM (
  ...
  group by all
  QUALIFY ROW_NUMBER() OVER (ORDER BY sum(trading_ngr_inc_jp_wins) + sum(nvl(jp_wins,0)) DESC) <= 10
) "Custom SQL Query"
