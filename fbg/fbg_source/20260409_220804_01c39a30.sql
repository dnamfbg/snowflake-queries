-- Query ID: 01c39a30-0212-6e7d-24dd-0703193f9843
-- Database: FBG_SOURCE
-- Schema: unknown
-- Warehouse: TABLEAU_XL_PROD
-- Executed: 2026-04-09T22:08:04.591000+00:00
-- Elapsed: 68653ms
-- Environment: FBG
...
FROM (
  ...
  group by all
  QUALIFY ROW_NUMBER() OVER (ORDER BY sum(trading_ngr_inc_jp_wins) + sum(nvl(jp_wins,0)) ASC) <= 10
) "Custom SQL Query"
