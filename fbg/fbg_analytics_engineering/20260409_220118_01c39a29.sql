-- Query ID: 01c39a29-0212-644a-24dd-0703193e0dd3
-- Database: FBG_ANALYTICS_ENGINEERING
-- Schema: unknown
-- Warehouse: BI_L_WH
-- Executed: 2026-04-09T22:01:18.850000+00:00
-- Elapsed: 436242ms
-- Environment: FBG
...
FROM (
  ...
  SELECT 
  DATE(date_trunc('week',CONVERT_TIMEZONE('America/Anchorage','America/New_York',session_end_time_alk))) as DATE,
  ...
  sbactives as (
  select
  date(date_trunc('week',CONVERT_TIMEZONE('UTC','America/New_York',a.placed_time))) as day,
  ...
  contactsetup as (
  select
  DATE_TRUNC('week',CONVERT_TIMEZONE('UTC','America/New_York',to_timestamp(concat(yearr,'-',monthh,'-',dayy,' ',clockhour,':',clockmin,':',clocksec),'YYYY-MM-DD HH:MI:SS'))) as day,
  date(day) as DDate,
  ...
  fraudsetup as (
  select date_trunc('week',day) as date,
