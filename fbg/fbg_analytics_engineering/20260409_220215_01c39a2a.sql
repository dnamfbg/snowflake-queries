-- Query ID: 01c39a2a-0212-67a8-24dd-0703193e39e3
-- Database: FBG_ANALYTICS_ENGINEERING
-- Schema: unknown
-- Warehouse: BI_L_WH
-- Executed: 2026-04-09T22:02:15.469000+00:00
-- Elapsed: 455615ms
-- Environment: FBG
...
FROM (
  ...
  SELECT 
  DATE(date_trunc('month',CONVERT_TIMEZONE('America/Anchorage','America/New_York',session_end_time_alk))) as DATE,
  ...
  sbactives as (
  select
  date(date_trunc('month',CONVERT_TIMEZONE('UTC','America/New_York',a.placed_time))) as day,
  ...
  contactsetup as (
  select
  DATE_TRUNC('month',CONVERT_TIMEZONE('UTC','America/New_York',to_timestamp(concat(yearr,'-',monthh,'-',dayy,' ',clockhour,':',clockmin,':',clocksec),'YYYY-MM-DD HH:MI:SS'))) as day,
  date(day) as DDate,
  ...
  fraudsetup as (
  select date_trunc('month',day) as date,
