-- Query ID: 01c39a1b-0212-6b00-24dd-0703193acc17
-- Database: FBG_ANALYTICS_DEV
-- Schema: unknown
-- Warehouse: BI_SER_M_WH
-- Executed: 2026-04-09T21:47:10.136000+00:00
-- Elapsed: 42273ms
-- Environment: FBG
...
FROM (
  ...
  DATE(CONVERT_TIMEZONE('UTC','America/Anchorage', bets.settlement_time)) AS event_DATE,
  ...
  fbg_source.osb_source.bets inner join fbg_source.osb_source.bet_parts on (bets.id = bet_parts.bet_id)
                             ...
                             left outer join fbg_analytics.trading.nba_game_weeks_ egw on DATE(CONVERT_TIMEZONE('UTC','America/Anchorage', bets.settlement_time)) between egw.week_start_date and egw.week_end_date and case when lower(bet_parts.comp) like '%nba%' then 1 else 0 end = egw.comp_join_flag
  ...
  accounts.test = 0
  and
  lower(bet_parts.sport) = 'basketball'
  AND
  case when lower(bet_parts.comp) like '%nba%' AND lower(bet_parts.comp) not like '%wnba%' then 1 else 0 end = 1
  and
  bets.status NOT IN ('REJECTED','VOID')
  and
  DATE(CONVERT_TIMEZONE('UTC','America/Anchorage', bets.settlement_time)) >= '2025-10-01'
  and
  CASE
