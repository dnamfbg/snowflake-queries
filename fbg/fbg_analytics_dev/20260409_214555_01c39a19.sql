-- Query ID: 01c39a19-0212-6cb9-24dd-0703193ab80b
-- Database: FBG_ANALYTICS_DEV
-- Schema: unknown
-- Warehouse: BI_SER_M_WH
-- Executed: 2026-04-09T21:45:55.540000+00:00
-- Elapsed: 67900ms
-- Environment: FBG
...
FROM (
  ...
  fbg_source.osb_source.bets inner join fbg_source.osb_source.bet_parts on (bets.id = bet_parts.bet_id)
                             ...
                             left outer join fbg_analytics.trading.nba_game_weeks_ egw on DATE(CONVERT_TIMEZONE('UTC','America/Anchorage', bet_parts.event_time)) between egw.week_start_date and egw.week_end_date and case when lower(bet_parts.comp) like '%nba%' then 1 else 0 end = egw.comp_join_flag
  ...
  and
  DATE(CONVERT_TIMEZONE('UTC','America/Anchorage', bet_parts.event_time)) >= '2025-10-01'
  and
