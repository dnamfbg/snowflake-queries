-- Query ID: 01c399fb-0212-67a9-24dd-070319330e73
-- Database: FBG_ANALYTICS_DEV
-- Schema: ALVIN_CHAI
-- Warehouse: BI_L_WH
-- Executed: 2026-04-09T21:15:05.772000+00:00
-- Elapsed: 729ms
-- Environment: FBG

WITH all_customer_mart_models AS (
...
exclude_models AS (
    SELECT model_name FROM (VALUES
        ...
        ('trading_legs'),('trading_wagers'),('sporting_events'),('promotions'),
        ('stg_fancash_stake_amounts'),('stg_trading_sporting_event_results_summary_parse'),
        ('itm_resettled_wagers'),('itm_boost_token_payout'),('stg_last_updated_leg_wager'),
        ('stg_wager_leg_results'),('stg_trading_wagers'),('stg_trading_sporting_event_results_score_summary_parse'),
        ('itm_restricted_handle'),('stg_estimated_expected_margin'),('stg_trading_accounts'),
        ('itm_boost_market_payout'),('stg_trading_displayed_events'),
        ('stg_trading_sporting_event_results_summary'),('stg_trading_legs'),('stg_trading_promotions'),
        ('itm_boost_payout'),('dim_trading_jurisdictions'),
        ('stg_trading_sporting_event_results_score_summary'),
        ('stg_trading_legs_sport_category'),
        ('stg_trading_sporting_special_event_details'),('stg_trading_event_event_history'),
        ('event_history'),('itm_non_boosted_selection_price'),('stg_trading_promotions_oddsboost'),
        ('stg_trading_sporting_special_level_event_details'),('stg_trading_sporting_event_details')
    ) AS t(model_name)
),
included_models AS (
    SELECT a.model_name
    FROM all_customer_mart_models a
    LEFT JOIN exclude_models e ON LOWER(a.model_name) = LOWER(e.model_name)
...
model_daily_cost AS (
    ...
    INNER JOIN included_models d ON LOWER(r.NAME) = LOWER(d.model_name)
    WHERE r.RUN_DATE_UTC >= DATEADD('day', -30, CURRENT_DATE())
    GROUP BY r.NAME, r.MATERIALIZATION, r.WAREHOUSE_SIZE
),
ranked AS (
    SELECT *,
        ROW_NUMBER() OVER (PARTITION BY NAME ORDER BY total_runs DESC, cost_per_run DESC) AS rn
    FROM model_daily_cost
),
per_model AS (
    SELECT NAME, MATERIALIZATION, WAREHOUSE_SIZE, cost_per_run, avg_runtime_min
    FROM ranked WHERE rn = 1
)
SELECT
    COUNT(*) AS models_included,
    (SELECT COUNT(DISTINCT model_name) FROM exclude_models) AS models_excluded,
    ROUND(SUM(cost_per_run), 2) AS estimated_cost_per_dag_run,
    ROUND(SUM(CASE WHEN MATERIALIZATION IN ('table','incremental') THEN cost_per_run ELSE 0 END), 2) AS materialized_cost,
    ROUND(SUM(CASE WHEN MATERIALIZATION = 'view' THEN cost_per_run ELSE 0 END), 4) AS view_cost,
    ROUND(SUM(avg_runtime_min), 2) AS total_runtime_min_sequential
FROM per_model
