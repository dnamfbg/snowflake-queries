-- Query ID: 01c39a2a-0212-6e7d-24dd-0703193e4b5b
-- Database: FBG_ANALYTICS_DEV
-- Schema: PUBLIC
-- Warehouse: TABLEAU_XL_PROD
-- Executed: 2026-04-09T22:02:07.668000+00:00
-- Elapsed: 182830ms
-- Environment: FBG

SELECT "Custom SQL Query"."CONTACTRATE" AS "CONTACTRATE",
  "Custom SQL Query"."CX" AS "CX",
  "Custom SQL Query"."DAY" AS "DAY",
  "Custom SQL Query"."TOTALUNIQUE" AS "TOTALUNIQUE"
FROM (
  ...
  where created >= '2024-08-01' 
  --and v.coded_total_tier IN ('Tier 4', 'Tier 5') 
  and status != 'DEPOSIT_ABANDONED' and payment_brand not in ('CASHATCAGE','PAYSAFECASH','TERMINAL')
  ORDER BY date DESC),
  ...
  select
  dayvipcr as day, cxvip as cx, total_uniqueVIP as totalunique,
  cxvip/total_uniqueVIP as contactrate
  from final1 f
  left join final2 g
