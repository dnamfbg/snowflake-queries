-- Query ID: 01c399f4-0212-67a8-24dd-070319319a0f
-- Database: FBG_ANALYTICS_DEV
-- Schema: ALVIN_CHAI
-- Warehouse: BI_L_WH
-- Executed: 2026-04-09T21:08:23.714000+00:00
-- Elapsed: 543ms
-- Environment: FBG
...
SELECT
    COUNT(*) AS total_models_with_data,
    ROUND(SUM(cost_per_run), 2) AS estimated_total_cost_per_dag_run,
    ROUND(SUM(avg_runtime_min), 2) AS total_runtime_minutes_sequential,
    ROUND(SUM(CASE WHEN MATERIALIZATION IN ('table','incremental') THEN cost_per_run ELSE 0 END), 2) AS materialized_models_cost,
    ROUND(SUM(CASE WHEN MATERIALIZATION = 'view' THEN cost_per_run ELSE 0 END), 4) AS view_models_cost
FROM per_model
