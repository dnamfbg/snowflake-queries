-- Query ID: 01c39a25-0112-6b51-0000-e307218c00ae
-- Database: unknown
-- Schema: unknown
-- Warehouse: FDE_FES_ANALYST_XL_WH
-- Executed: 2026-04-09T21:57:07.024000+00:00
-- Elapsed: 5562ms
-- Environment: FES
...
, fbg_flags as (
    select 
        f.*,
        case 
            when first_osb_state_flag = 1 and 
            (fbg_reg_ts_alk is null or fanapp_install_ts_alk < fbg_reg_ts_alk) or (fbg_ftu_ts_alk is null or fbg_ftu_ts_alk > fanapp_install_ts_alk) then 1 else 0 end as FTU_Eligible_install_flag,
        case 
...
select 
    * 
from fbg_flags
    where fanapp_install_ts_alk < fbg_reg_ts_alk
    or fbg_reg_ts_alk is null;
