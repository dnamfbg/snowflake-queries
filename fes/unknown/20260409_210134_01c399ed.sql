-- Query ID: 01c399ed-0112-6ccc-0000-e3072189f002
-- Database: unknown
-- Schema: unknown
-- Warehouse: FDE_FES_ANALYST_XL_WH
-- Executed: 2026-04-09T21:01:34.989000+00:00
-- Elapsed: 13983ms
-- Environment: FES
...
, amplitude_generated_ftus as (
select 
    pfi.private_fan_id, 
    amplitude_fanapp_tenant_fan_id, 
    ftp_first_game_ts_utc,
    ftu_source,
    fanapp_ftu_attribution
from fangraph.private_fan_id.pfi_customer_mart pfi
    inner join amplitude_only_ids m
        on fanapp_tenant_fan_id = amplitude_fanapp_tenant_fan_id
    left join fanapp.reporting.fbg_affiliate_ftus ftu
        on pfi.private_fan_id = ftu.private_fan_id
        )
select fanapp_ftu_attribution, count(distinct private_fan_id) from amplitude_generated_ftus group by all;
